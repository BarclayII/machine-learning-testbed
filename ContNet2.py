...
class DynNet:
    ...
    def __init__(self, params=None, **options):
        ...
        self._add_fc("loc_in", self.opts["location_dims"] * self.opts["historylength"], self.opts["gn_first_dims"])
    ...
    def step_lstm(self, location, glimpses, prev_candt1, prev_outpt1, prev_candt2, prev_outpt2):
        ...
        return loc_out, next_candt1, core_out1, next_candt2, core_out2
    ...
    def train(self, env):
        ...
        try:
            ...
            for gamenum in xrange(0, self.opts["training_size"]):
                ...
                locs = [NP.zeros((self.opts["location_dims"],)) for i in range(1, self.opts["historylength"])]
                loc_in = []
                ...
                loc_out = [NP.zeros((self.opts["location_dims"],)) for i in range(1, self.opts["historylength"])]
                ...
                real_out = [location_expand(NP.array([env._ball.posIX, env._ball.posIY]), env.size(), self.opts["location_dims"])]
                reward = [0.]
                chosen_loc = [location_expand(NP.asarray((env._ball.posIX, env._ball.posIY)), env.size(), self.opts["location_dims"])]
                img_ca = []
                img_env = []
                img_glm = []
                cost = [0.]
                ...
                # print env.done(), env._ball.posIX, env._ball.posIY
                ...
                while not env.done():
                    ...
                    if (self.opts["learningmodel"] == 'supervised') or (self.opts["learning_mode"] == 'test'):
                        ...
                        ro = location_expand(NP.array([env._ball.posIX, env._ball.posIY]), env.size(), self.opts["location_dims"])
                        if self.opts["learning_mode"] == 'test':
